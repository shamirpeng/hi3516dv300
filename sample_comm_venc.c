...
1. 功能概述
这段代码定义了一个名为SAMPLE_COMM_VENC_GetFilePostfix的函数，
其主要功能是根据输入的视频编码负载类型（PAYLOAD_TYPE_E类型的枚举值）来获取对应的文件后缀名，
并将其复制到传入的字符指针szFilePostfix所指向的内存空间中。如果输入的负载类型不匹配已定义的几种类型，
函数会打印错误信息并返回错误标识HI_FAILURE，若匹配成功并完成后缀名复制，则返回HI_SUCCESS
******************************************************************************/
